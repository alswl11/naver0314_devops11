...
public class Ex1_ClassShop {
	...
	public static void writeShop(Shop [] myShop) {
		...
		for(int i = 0; i<myShop.length; i++) {
			System.out.println(i + "\t" + myShop[i].getSang() + "\t" + myShop[i].getSu() + "\t" + myShop[i].getDan() + "\t"+ myShop[i].getTot());
		}
...
//		myShop[0] = new Shop(); // default는 0번째로 설정 
...
//		myShop[3] = new Shop("사과", 5, 3000);
		...
		showTitle(); // 제목 출력하는 static 메서드 < 번호 상품 수량 단가 총 금액 > 
		writeShop(myShop);
